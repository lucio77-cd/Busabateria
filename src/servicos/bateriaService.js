...
// lojaId = uid do usuário logado como loja (simplificação: a conta da loja
// É a própria referência, sem precisar de um documento "lojas" separado por enquanto)
...
export async function listarBateriasPorLoja(lojaId) {
  const q = query(
    ...
    limit(50) // economia — nunca sem limite
  );
...
export function criarBateria(lojaId, dados) {
  return addDoc(collection(db, "baterias"), {
    lojaId,
    marca: dados.marca,
    modelosCompativeis: dados.modelosCompativeis, // array de strings
